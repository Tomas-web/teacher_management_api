...
import management.teacher_management_api.drivers.api.payloads.calls.ValidateCallRoomRequest;
...
public class CallController {
    ...
    @PostMapping("/users/{user_id}/call-room/validate")
    public ResponseEntity<Void> validateCallRoom(
            @PathVariable("user_id") String targetId,
            @RequestBody ValidateCallRoomRequest body) {
        ...
        try {
            val result =
                    validateCallRoomUsecase.execute(
                            userId,
                            Long.parseLong(body.getCallerId()),
